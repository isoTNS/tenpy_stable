...
class PlaneWaveExcitations(Algorithm):
    ...
    class Unaligned_Effective_H(NpcLinearOperator):
        __init__(self, outer, ALs, ARs, VLs, LW, RW, Ws, p, chi, d=2):
            outer.n
            self.ALs = ALs * n
            self.ARs = ARs * n
            self.Ws = Ws * n
        ...
        def matvec(self, vec):
            ...
            for j in range(L):
                ...
                if N_X == 0:
                    ...
                    ket = self.ALs[:j] + [self.VLs[j]] + Xs
                ket = ket[:L] #could be incomplete
                ket = ket + self.ARs[len(ket):L] #if leaks, ARs doesn't get appended
                assert len(ket) == L
                Ws = self.Ws[:L]
                bra = self.ARs[:L]
                R0 = TR_general(ket, bra, R0, Ws=Ws)

                R0 = self.outer.infinite_sum_TLR(R0, self.p)
            ...
            for i in range(L):
                ...
                for j in range(L):
                    ...
                    if N_X == 0:
                        B = npc.tensordot(self.VLs[j], vec[j], axes=(['vR'], ['vL']))
                        ket_n += [B] + self.ARs[j+1+N_X:]
                    else:
                        ket_n += [self.VLs[j]] + Xs + self.ARs[j+1+N_X:]
                    assert len(ket_n) == n*L
                    #-------get L------------
                    bra = self.ALs[:i+L] + [VLs[i]]
                    ket = ket_n[:i+L+1]
                    mpo = self.Ws[:i+L+1]
                    LB  = LT_general(ket, bra, self.LW, Ws=mpo)
                    #-------get middle ket and mpo-------
                    ket_mid = ket_n[L+i+1:L+l] + self.ARs[:l-L]
                    mpo_mid = self.Ws[L+i+1:L+l] + self.Ws[:l-L]
                    assert len(ket_mid) == N_X and len(mpo_mid) == N_X
                    #-------get R--------------
                    R       = self.GS_env.get_RP((l-1)%L)
                    #-------get action--------------
                    total_vec[i] += form_X(ket_mid, LB, R, Ws=mpo_mid) * np.exp(-1.0j*self.p*L)
            #Term 2
            for j in range(L):
                ...
                ket = ket_n
                mpo = self.Ws
                L0  = LT_general(ket, bra, self.LW, Ws=mpo)
                L0  = self.outer.infinite_sum_TRL(L0, self.p)
                for i in range(L):
                    #-------get L------------
                    ket = self.ARs[:i+1]
                    bra = self.ALS[:i] + [VLs[i]]
                    mpo = self.Ws[:i+1]
