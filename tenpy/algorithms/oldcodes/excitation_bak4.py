...
class PlaneWaveExcitations(Algorithm):
    def __init__(self, psi, model, options, **kwargs):
        ...
        self.n = 1 # number of unit cells one works with in the PW eigenvalue problem
    ...
    class Aligned_Effective_H(NpcLinearOperator):
        ...
        def matvec(self, vec):
            ...
            for i in range(self.outer.L): #compute the environment of X_i; i is in the lower
                if i > 0:
                    for j in range(0, i):
                        '''
                                    -----__ket__-------
                                    |    |  |  |      |
                        left term = LB---W--W--W------R
                                    |    |  |  |      |
                                    ----           ----
                                        i+1    i+NX      ,   illustration for N_X = 3
                        '''
                        #leaking criterion: i+1+N_X > L
                        l_leak = i+1+self.N_X - self.outer.L

#                          QR Xj=vec[j] into Xs
                        Xs = []
                        #-------get LB------------
                        bra = self.ALs[0:i] + [self.VLs[i]]
                        mpo = self.Ws[0:i+1]
#                          ket = self.ALs[0:j] + [self.VLs[j]] + Xs[:i-j] #if i-j > len(Xs), python uses Xs
                        B = npc.tensordot(self.VLs[j], vec[j], axes=(['vR'], ['vL']))
                        ket = self.ALs[0:j] + [B] + Xs[:i-j] #if i-j > len(Xs), python uses Xs
                        ket = ket + self.ARs[len(ket):i+1]
                        assert len(ket) == len(bra) and len(bra) == len(mpo)
                        LB  = LT_general(ket, bra, self.LW, Ws=mpo)
                        #-------get middle ket and mpo-------
                        ket_meat = Xs[i-j:] #could be empty when i-j >= N_X
                        while len(ket_meat) < self.N_X:
                            i_AR = i + 1 + len(ket_meat)
                            ket_meat.append(ARs[i_AR % self.outer.L])
                        assert len(ket_meat) == self.N_X
                        mpo_meat = self.Ws[i+1:i+1+self.N_X] #could be incomplete
                        if l_leak > 0:
                            mpo_meat = mpo_meat + self.Ws[:l_leak]
                        assert len(mpo_meat) == self.N_X
                        #-------get R--------------
                        R = self.outer.GS_env.get_RP(i)
#                          if l_leak > 0:
#                              R = self.outer.GS_env.get_RP(l_leak-1)
#                          else:
#                              ket = self.ARs[i+1+N_X:] + self.ARs
#                              mpo = self.Ws[i+1+N_X:] + self.Ws
#                              bra = self.ARs[i+1+N_X:] + self.ARs
#                              R = TR_general(ket, bra, self.RW, Ws=mpo)
                        #-------get action--------------
                        total_vec[i] += form_X(ket_meat, LB, R, Ws=mpo_meat)
                if i < self.outer.L-1:
                    for j in range(i+1, self.outer.L-1):
                    '''
                                    -----__ket__-------
                                    |    |  |  |      |
                        left term = LB---W--W--W------R
                    ...
                    for j in range(self.outer.L-1, i, -1):
                        B = npc.tensordot(self.VLs[j], vec[j], axes=(['vR'], ['vL']))
                        if j < self.outer.L - 1:
                            ...
                            RB = TR_general([B], [self.ARs[j]], self.outer.GS_env.get_RP(j), Ws=[self.Ws[j]])
                    LP = LT_general([self.ALs[i]], [self.VLs[i]], self.outer.GS_env.get_LP(i), Ws=[self.Ws[i]])
                    total_vec[i] += npc.tensordot(LP, RB, axes=(['vR', 'wR'], ['vL', 'wL']))
                #-----------------------------------------
                B = npc.tensordot(self.VLs[i], vec[i], axes=(['vR'], ['vL']))
                LP = LT_general([B], [self.VLs[i]], self.outer.GS_env.get_LP(i), Ws=[self.Ws[i]])
                total_vec[i] += npc.tensordot(LP, self.outer.GS_env.get_RP(i), axes=(['vR', 'wR'], ['vL', 'wL']))
                '''
                    ---B--------
                    |  |       |
                 LW_i--W-------RW_i
