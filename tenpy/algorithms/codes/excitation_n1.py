        ...
        def matvec(self, vec):
            ...
            for j in range(L):
                ...
                if N_X == 0:
                    ...
                    ket_n = self.ALs[:j] + [self.VLs[j]] + Xs
                ket_n = ket_n + self.ARs[l:]
                assert len(ket_n) == L

                R1  = TR_general(ket_n, self.ARs, R0, Ws=self.Ws)
                ...
                for i in range(L):
                    ...
                    #Term 2
                    #-------get LB------------
                    ket = self.ALs[:i+1] #FIXME Fix this in excitation_new.py
                    mpo = self.Ws[:i+1]
                    bra = bra_n[:i+1]
                    LB  = LT_general(ket, bra, self.LW, Ws=mpo)
                    #-------get middle ket and mpo-------
                    ket_mid = self.ALs[i+1:i+N_X+1] #FIXME Fix this in excitation_new.py
                    mpo_mid = self.Ws[i+1:i+N_X+1]
                    #-------get R--------------
                    ket = self.ALs[i+N_X+1:] #FIXME Fix this in excitation_new.py
                    mpo = self.Ws[i+N_X+1:]
                    bra = bra_n[i+N_X+1:]
