        ...
        for qindices in res._iter_all_blocks():
            ...
            if np.all(res._get_block_charge(qindices) == qtotal):
                ...
                if raise_wrong_sector:
                    raise ValueError("wrong sector with non-zero entries: {}".format(np.abs(data_flat[sl])))
    ...
    def from_ndarray_YW(cls,
        ...
        for qindices in res._iter_all_blocks():
            ...
            if np.all(res._get_block_charge(qindices) == qtotal):
                ...
                off_diagonal_error += np.sum(np.abs(data_flat[sl])**2)
                if np.any(np.abs(data_flat[sl]) > cutoff):
                    if raise_wrong_sector:
                        raise ValueError("wrong sector with non-zero entries: {}".format(np.abs(data_flat[sl])))
                    if warn_wrong_sector:
                        warnings.warn("flat array has non-zero entries in blocks "
                                      "incompatible with charge",  stacklevel=2)
                        if verbose:
                            print('WARN OD_err:', np.sqrt(np.sum(np.abs(data_flat[sl])**2)))
        res._data = data
        res._qdata = np.array(qdata, dtype=np.intp, order='C').reshape((len(qdata), res.rank))
        res._qdata_sorted = True
        res.test_sanity()
        return res, np.sqrt(off_diagonal_error)
...
#          print("labels:", labels)
    ...
    def __str__(self):
        ...
        if np.prod(self.shape) < 16:
            res.append(str(self.to_ndarray()))
        res.append('qtotal: '+str(self.qtotal))
        res.append('norm^2: '+str(self.norm()**2))
        res.append('>')
        return '\n'.join(res)
...
#                    qconj_i = +1  # will change in future to
                    qconj_i = self.get_leg(combine_legs[i][0]).qconj #Why future? I'm changing it now! - YW
